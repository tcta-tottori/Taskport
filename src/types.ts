 ...
 * 実行ログ（開始・一時停止・終了）
 *
 * **「いつからいつまで」を持つのはここ。合計は台帳へ書き戻す。**
 *   区間（何時何分から何分間）… この実行ログ
 *   合計（かかった時間）      … タスクは台帳の `actualMin`、予定はログだけ
 *
 * 台帳の `startedAt` は「いま動いている印」で、止めると消える。
 * それだけだと**止めたあとに「何時にやったか」が消える**ので、
 * 時間帯の色分け（分析）と日報の枠決めのために区間をここに残す（v1.22.0）。
 *
 * 記録は**書き換えない**（止めた区間の時刻を後から動かすと、実績が実績で
 * なくなる。直すのは台帳側の合計。消したいときは記録ごと消す）。
 *
 * タスクと予定を同時に動かせる。電話を受けながら伝票を打つ、が実際に起きる。
 * ------------------------------------------------------- */

/** 実行の対象。台帳のタスクか、予定の1回ぶんか */
